...
from preprocess_data_fixed_length import process
...
def load_data(config=None):
    ...
    test_file = os.path.join(config.data_save_path,'test_{}.json'.format(config.max_length))
...
def test():
    ...
    loss_file = os.path.join(config.data_save_path,'loss_{}.pkl'.format(config.max_length))
